...
import { loadData, saveData, signInWithGoogle, logOut, onAuthChange, getGoogleRedirectResult } from "./firebase.js";
...
export default function App() {
  ...
  useEffect(()=>{
    if(user) load().then(d=>{ setTeams(d||DEFAULT_TEAMS); setLoading(false); });
  },[user]);
  ...
  if(authLoading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0d0f18",color:"#6b7280"}}>Cargando...</div>;

  if(!user) return <GoogleLogin onLogin={()=>signInWithGoogle().catch(console.error)}/>;

  if(loading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0d0f18",color:"#6b7280"}}>Cargando datos...</div>;
  ...
  return <><style>{css}</style><div className="app">
    ...
    {view==="team"        && team && <TeamView team={team} teams={teams} tab={tab} setTab={setTab} updateTeam={updateTeam} logout={()=>{logOut();setView("login");setStep("role");}}/>}
    {view==="coordinator" && <CoordView teams={teams} persist={persist} updateTeam={updateTeam} logout={()=>{logOut();setView("login");setStep("role");}}/>}
  </div></>;
}
...
function GoogleLogin({onLogin}){
  return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0d0f18",padding:24}}>
    <div style={{background:"#13161f",border:"1px solid #252836",borderRadius:22,padding:"44px 36px",width:"100%",maxWidth:380,textAlign:"center"}}>
      <div style={{width:56,height:56,background:"#2563eb",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 24px"}}>⚽</div>
      <div style={{fontSize:22,fontWeight:700,marginBottom:6}}>Canet F.C.</div>
      <div style={{fontSize:13,color:"#6b7280",marginBottom:32}}>Gestión de Asistencia</div>
      <button onClick={onLogin} style={{width:"100%",background:"white",border:"none",borderRadius:12,padding:"14px 20px",fontSize:15,fontWeight:600,color:"#1a1a1a",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:12,transition:"all .15s"}}
        onMouseOver={e=>e.currentTarget.style.background="#f0f0f0"}
        onMouseOut={e=>e.currentTarget.style.background="white"}>
        <svg width="20" height="20" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.6 20H24v8h11.3C33.7 32.8 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.8 1.1 7.9 3l5.7-5.7C34 6.1 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20c11 0 20-8 20-20 0-1.3-.1-2.7-.4-4z"/><path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.5 16 19 12 24 12c3 0 5.8 1.1 7.9 3l5.7-5.7C34 6.1 29.3 4 24 4c-7.7 0-14.4 4.3-17.7 10.7z"/><path fill="#4CAF50" d="M24 44c5.2 0 9.9-1.9 13.5-5l-6.2-5.2C29.4 35.5 26.8 36 24 36c-5.2 0-9.7-3.1-11.3-7.6l-6.6 5.1C9.5 39.6 16.3 44 24 44z"/><path fill="#1976D2" d="M43.6 20H24v8h11.3c-.8 2.3-2.3 4.3-4.3 5.8l6.2 5.2C41 35.8 44 30.3 44 24c0-1.3-.1-2.7-.4-4z"/></svg>
        Entrar con Google
      </button>
      <div style={{fontSize:11,color:"#4b5563",marginTop:16}}>Solo usuarios autorizados por el club</div>
    </div>
  </div>;
}
