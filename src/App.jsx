...
// ─── STORAGE ─────────────────────────────────────────────────
const load = async(k,sh=true)=>{ try{const r=await window.storage.get(k,sh);return r?JSON.parse(r.value):null;}catch{return null;}};
const save = async(k,v,sh=true)=>{ try{await window.storage.set(k,JSON.stringify(v),sh);}catch{}};
...
export default function App() {
  ...
  useEffect(()=>{ load("canet_v4").then(d=>{ setTeams(d||DEFAULT_TEAMS); setLoading(false); }); },[]);
